...
class PathNode(LifecycleNode):
    ...
    Lifecycle transitions:
      - on_configure  : declare parameters, initialize state variables, create
                        subscriptions (data can be received in inactive state).
      - on_activate   : create publishers and timer so the node starts doing work.
      - on_deactivate : destroy publishers and timer; subscriptions stay alive.
      - on_cleanup    : destroy subscriptions and reset all state.
      - on_shutdown   : same clean-up regardless of which state we are in, can NOT be revived.
    """
    ...
    def on_activate(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        Create publishers and the periodic timer so work begins.
        """
        ...
        # Let the base class activate the lifecycle publishers
        return super().on_activate(state)

    def on_deactivate(self, state: LifecycleState) -> TransitionCallbackReturn:
        """
        Called when transitioning from Active → Inactive.
        Destroy the timer and publishers; subscriptions are kept alive.
        """
        ...
        # Let the base class deactivate remaining managed entities
        return super().on_deactivate(state)

    def on_cleanup(self, state: LifecycleState) -> TransitionCallbackReturn:
        """
        Called when transitioning from Inactive → Unconfigured.
        Destroy subscriptions and reset all state so the node can be
    ...
    # ------------------------------------------------------------------
    # Subscription callbacks  (unchanged logic)
    # ------------------------------------------------------------------
    ...
    def odom_callback(self, msg):
        if self.track_received:
            ...
            self.yaw = np.arctan2(
                2 * (orientation_q.w * orientation_q.z - orientation_q.x * orientation_q.y),
    ...
    # ------------------------------------------------------------------
    # Publishing helpers  (unchanged logic)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Timer callback  (unchanged logic)
    # ------------------------------------------------------------------
    ...
    def timer_callback(self):
        ...
        if self.go_msg.mission == "trackdrive" and self.track_received:
            ...
            np_array_path, np_array_path_concatenated = self.planner.get_interpolated_path(
                self.obstacle_numpy_array, self.car_pose
            )
